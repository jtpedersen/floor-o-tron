"""
Usage:
  bump.py <directory> --major "Release notes for major version"
  bump.py <directory> --minor "Release notes for minor version"
  bump.py <directory> --patch "Release notes for patch version"
...
def update_changelog(directory, new_version, release_notes):
    ...
    new_entry = f"## [{new_version}] - {sys.argv[-1]}\n### Changes\n- {release_notes}\n\n"
    with open(changelog_path, 'w') as f:
...
def commit_and_tag_repo(directory, new_version):
    ...
    try:
        subprocess.run(["git", "add", os.path.join(directory, CONFIG_FILE), os.path.join(directory, DOCKER_FILE), os.path.join(directory, CHANGELOG_FILE)], check=True)
...
def main():
    ...
    if args['--major']:
        ...
        release_notes = args['--major']
    elif args['--minor']:
        new_version = increment_version(current_version, 'minor')
        release_notes = args['--minor']
    elif args['--patch']:
        new_version = increment_version(current_version, 'patch')
        release_notes = args['--patch']
    else:
